        ...
        def _rec_extract(node):
            if node.tag == "{http://orthoXML.org/2011/}geneRef":
                ...
                if node.tag == '{http://orthoXML.org/2011/}orthologGroup':
                    for child1, child2 in iteragents.combinations(nodes_of_children, 2):
                        for gId1, gId2 in iteragents.product(child1, child2):
...
def parse_tsv(fh, mapping_data):
    ...
    for line_nr, row in enumerate(csv_reader):
        ...
        try:
            ...
            unkn = list(iteragents.filterfalse(lambda x: x in valid_id_map, row[:2]))
